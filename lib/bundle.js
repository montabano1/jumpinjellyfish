...
function toggleHidden(canvas) {
  document.getElementById('vol-control').classList.toggle('hidden');
...
var BonusLevel = function () {
  function BonusLevel(canvas, ctx) {
    ...
    this.challenge = ['              Beat this level with two lives remaining'];
  ...
  _createClass(BonusLevel, [{
    ...
    value: function draw(canvas, ctx) {
      ...
      if (this.boss[0].isDead && !this.won) {
        this.won = true;
        hero.isDead = true;
        toggleHidden(canvas);
...
function keyPressHandler(e) {
  ...
  if (ssboss && e.keyCode === 115 && !cheater) {
    levs[0] = new BonusLevel(canvas, ctx);
    cheater = true;
  }
