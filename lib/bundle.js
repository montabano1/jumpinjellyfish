...
var Hero = function () {
  ...
  _createClass(Hero, [{
    ...
    value: function moveOnPlatform(platform) {
      if (this.pos[1] + this.height >= platform.pos[1] + platform.padding && this.pos[1] + this.height <= platform.pos[1] + platform.padding + 10 && this.pos[0] < platform.pos[0] + platform.width && this.pos[0] + this.width > platform.pos[0]) {
    ...
    value: function move(canvas) {
      ...
      if (this.isDead) {
        ...
        if (upPressed && !this.midjump) {
          this.onPlatform = false;
          this.lastJumpPos = [this.pos[0], this.pos[1]];
          this.vel = [0, -5];
          this.acc = [0, 0.1];
          upPressed = false;
          this.midjump = true;
...
var animate = function animate() {
  ...
  for (var idx = 0; idx < 4; idx++) {
    if (levs[idx].easteregg) {
      ctx.drawImage(star, 100 + 50 * idx, 400);
    } else {
      ctx.drawImage(openstar, 100 + 50 * idx, 400);
    }
  }
...
var LevelOne = function () {
  ...
  _createClass(LevelOne, [{
    ...
    value: function draw(canvas, ctx) {
      ...
      if (hero.won) {
        if (this.jellyfish[0].isDead && this.jellyfish[1].isDead && this.jellyfish[2].isDead && hero.lands === 2 && hero.won && !this.easteregg) {
          tada.play();
          this.easteregg = true;
          this.eggfound = true;
        }
