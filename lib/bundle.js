...
var BonusLevel = function () {
  ...
  _createClass(BonusLevel, [{
    ...
    value: function draw(canvas, ctx) {
      ...
      ctx.rect(320, 30, Math.max(this.boss[0].hits * 50, 0), 30);
      ...
      this.boss.forEach(function (boss) {
        ...
        if (_this.boss[0].children) {
          if (!_this.boss[0].isDead) {
            splat.play();
          }
          _this.boss[0].children = false;
          _this.jellyfish.push(new Jellyfish({ pos: [_this.boss[0].pos[0] + 20, 250], vel: [-1, -3], acc: [0, -0.2], width: 40, height: 40, bouncable: true }));
          _this.jellyfish.push(new Jellyfish({ pos: [_this.boss[0].pos[0] + 20, 250], vel: [1, -3], acc: [0, -0.2], width: 40, height: 40, bouncable: true }));
        }
      });
...
var CongratsLevel = function () {
  ...
  _createClass(CongratsLevel, [{
    ...
    value: function draw(canvas, ctx) {
      ...
      ctx.fillStyle = 'orange';
      ctx.fillText("  Get all 4 bonuses to reach the secret level!", 500 - hero.pos[0], 450);
    }
  }]);
...
var Hero = function () {
  ...
  _createClass(Hero, [{
    ...
    value: function bounceOffJellyfish(jellyfish) {
      if (!this.isDead && jellyfish.isBouncable && this.pos[1] + this.height > jellyfish.pos[1] + 5 && this.pos[1] + this.height < jellyfish.pos[1] + jellyfish.width * 2 && this.pos[0] > jellyfish.pos[0] - 15 && this.pos[0] < jellyfish.pos[0] + jellyfish.width * 1.5) {
        if (this.playbounce) {
          bounce.play();
        }
        this.vel = [0, -5];
    ...
    value: function killedByJellyfish(jellyfish) {
      if (this.pos[0] > jellyfish.pos[0] - jellyfish.width * 27 / 40 && this.pos[0] < jellyfish.pos[0] + jellyfish.width * 65 / 40 && this.pos[1] + this.height > jellyfish.pos[1] + jellyfish.height * 2 && this.pos[1] < jellyfish.pos[1] + jellyfish.height * 2) {
...
var Jellyfish = function () {
  ...
  _createClass(Jellyfish, [{
    ...
    key: 'move',
    value: function move(ctx) {
      if (this.pos[0] < 0 || this.pos[0] > canvas.width - this.width - 30) {
        this.isDead = true;
      }
...
function keyPressHandler(e) {
  ...
  if (bonus && e.keyCode === 111) {
    boss = true;
  }
  if (boss && e.keyCode === 115) {
    sboss = true;
  }
  if (ssboss && e.keyCode === 115 && !cheater) {
    levs[0] = new BonusLevel(canvas, ctx, lives);
    cheater = true;
  }
  ...
  if ((directions || bonus) && e.keyCode != 99 && e.keyCode != 98 && e.keyCode != 116) {
    coral.play();
    directions = false;
    bonus = false;
  }
  ...
  if (e.keyCode === 13 && levs[i].hero.isDead && lives > 0) {
  ...
  if (lives <= 0) {
    ...
    if (controls) {
      ...
      if (directions) {
        ...
        ctx.fillStyle = 'pink';
        ctx.fillText("Level Directions", 250, 65);
        ctx.font = "40px fantasy";
        ctx.fillText(levs[i].directions[0], 400, 130);
        for (var j = 1; j < levs[i].directions.length; j++) {
          ctx.fillText(levs[i].directions[j], 180, 130 + 60 * j);
        }
      }
      if (bonus) {
        ctx.fillStyle = 'rgba(0,0,0,0.7)';
      ...
      if (eggs === 4 && bonuslevel === false) {
        bonuslevel = true;
        levs[4] = new BonusLevel(canvas, ctx, lives);
      }
...
var LevelOne = function () {
  function LevelOne(canvas, ctx) {
    ...
    this.challenge = ['              Kill all jellyfish on your FIRST jump and win', '     (Press enter to continue)'];
...
var LevelThree = function () {
  function LevelThree(canvas, ctx) {
    ...
    this.ctx = ctx;
    this.directions = ['LEVEL 3:', 'Use all of your skills to beat the', '                       final stage!', '     (Press enter to continue)'];
...
var LevelTwo = function () {
  function LevelTwo(canvas, ctx) {
    ...
    this.ctx = ctx;
    this.directions = ['LEVEL 2:', 'Use the mouse to click and hold on', 'to a jellyfish to freeze it in place', '     (Press enter to continue)'];
...
var LevelZero = function () {
  function LevelZero(canvas, ctx) {
    ...
    this.ctx = ctx;
    this.directions = ["LEVEL 0:", "Use A or the Left arrow to move LEFT.", "Use D or the Right arrow to move RIGHT.", "Use W or the spacebar to JUMP.", "Try to get to the other platform!", '     (Press enter to continue)'];
