...
const animate = () => {
  ...
  if (controls) {
    ...
    if (directions) {
      ...
      ctx.fillStyle = 'pink';
      ctx.fillText("Directions", 360, 210);
      ctx.font = "24px fantasy";
      ctx.fillText(levs[0].directions[0], 440, 300);
      for(let i=1; i < levs[0].directions.length; i++) {
        ctx.fillText(levs[0].directions[i], 320, 300 + 40*i);
      }
      ctx.fillText('(Press enter or v to continue)', 330, 150);
    }
    if (levs[0].hero.won) {
      ctx.fillStyle = 'rgba(0,0,0,0.7)';
      ctx.fillRect(0,0,960,640);
